...
public class CompanyServiceImpl implements CompanyService {
    ...
    public String publishDriveRound(String driveId, DriveRoundDto dto, String companyId) {
        ...
        int page = 0;
        int size = 50;
        Page<Applications> result;
        List<Applications> eligibleApplications = new ArrayList<>();
        do {
            result = applicationRepository.findByDrive_DriveIdAndStatus(driveId, requiredStatus, PageRequest.of(page, size));
            eligibleApplications.addAll(result.getContent());
            page++;
    ...
    @Override
    public Page<DriveDto> getAllDrives(String companyId, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Drive> drives = driveRepository.findByCompany_CompanyId(companyId,pageable);
        return drives.map(DriveMapper::maptoDriveDto);
    }

    @Override
    public Page<DriveRoundDto> getAllRounds(String driveId, String companyId, int page, int size) {
        verifyDriveOwnership(driveId, companyId);
        Pageable pageable = PageRequest.of(page,size);
      Page<DriveRound> driveRounds = driveRoundRepository
                .findByDrive_DriveId(driveId,pageable);
        return driveRounds.map(DriveRoundMapper::mapToDriveRoundDto);
    }

    @Override
    public Page<ApplicationsDto> getAllApplications(
        ...
        Page<Applications> applicationsPage =
                applicationRepository.findByDrive_DriveId(driveId, pageable);

        if (applicationsPage.isEmpty()) {
            return Page.empty();
        }

        return applicationsPage.map(ApplicationsMapper::mapToApplicationDto);
    }

    @Override
    public Page<ApplicationRoundProjection> getApplicantsForDriveRound(
    ...
    public String filterTopKStudents(String driveId, Integer roundNo,
        ...
        Page<ApplicationRound> result;
        List<ApplicationRound> studentApplications = new ArrayList<>();

        do {
            result = applicationRoundRepository.findScoredStudentsOrderByScore(driveId, roundNo, PageRequest.of(page, size));
            studentApplications.addAll(result.getContent());
            page++;
        } while (result.hasNext());
    ...
    public String filterByCutOffMarks(String driveId, Integer roundNo,
        ...
        List<ApplicationRound> studentApplications = new ArrayList<>();
        do {
           result = applicationRoundRepository
                    .findScoredStudentsOrderByScore(driveId, roundNo,PageRequest.of(page, size) );
            studentApplications.addAll(result.getContent());
            page++;
        }while(result.hasNext());
    ...
    public Integer countFilterByCutOffMarks(String driveId, Integer roundNo, Double cutOffMarks, String companyId) {
        ...
        do{
            result=applicationRoundRepository.findScoredStudentsOrderByScore(driveId, roundNo,PageRequest.of(page, size) );
            applicationRounds.addAll(result.getContent());
            page++;
        }while(result.hasNext());
        int count = 0;
...
// private final CloudinaryService cloudinaryService;  ← add to constructor
    ...
    @Override
    public Long countActiveDives(String companyId) {
        return driveRepository.countActiveDrivesByCompany(companyId);
    }

    @Override
    public Long totalDives(String companyId) {
        return driveRepository.countDrivesByCompany(companyId);
    }
    @Override
    public String closeDrive(String companyId,String driveId) {
        Drive drive = driveRepository. findByDriveIdAndCompanyCompanyIdAndIsActiveTrue(companyId,driveId)
                .orElseThrow(null);
        drive.setIsActive(false);
