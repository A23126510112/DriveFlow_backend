...
public class AdminServiceImpl implements AdminService {
    ...
    @Override
    public Page<StudentProfileDto> getAllProfiles(int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        return studentProfileRepository.findAll(pageable).map(StudentProfileMapper::maptoStudentProfileDto);
    }
    @Override
    ...
    public Page<DriveDto> getAllDrives(String companyId, int page, int size) {
        ...
        Page<Drive> drives = driveRepository
    ...
    public Page<CompanyDto> getAllCompanies(int page, int size) {
        ...
        return companies.map(company -> CompanyMapper.mapCompanyToDto(company));
    }

    @Override
    public Page<ApplicationsDto> getAllApplicationsForaStudent(String rollNo, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Applications> applicationsPage =
                applicationRepository.findByStudent_RollNo(rollNo, pageable);
    ...
    @Override
    public Page<DriveRoundDto> getAllRounds(String driveId, int page, int size){
        Pageable pageable = PageRequest.of(page, size);
    ...
    public Page<ApplicationsDto> getAllApplications(
            String driveId, int page, int size) {
    ...
    @Override
    public Page<ApplicationRoundProjection> getApplicantsForDriveRound(String driveId, Integer roundNo, int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
    ...
    @Override
    public Page<DriveDto> viewAllActiveDrives(int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Drive> drives=driveRepository.findAllByIsActive(true,pageable);
        return drives.map(DriveMapper::maptoDriveDto);
    }
    ...
    public Long countActiveDrives() {
        return driveRepository.findAllByIsActive(true).stream().count();
    }
...
// private final CloudinaryService cloudinaryService;  ← add to constructor
    ...
    @Override
    public boolean isEligibilityPublished(String driveId) {
        Drive drive=driveRepository.findByDriveId(driveId).orElseThrow(() -> new ResourceNotFoundException("drive not found"));
        if(drive.getIsActive()!=null){
            return true;
        }
        return false;
    }
