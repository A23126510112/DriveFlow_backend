...
public class CompanyController {
    ...
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<ApiResponse<Page<DriveDto>>> getAllDrives(
            @AuthenticationPrincipal Company company,
    ...
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<ApiResponse<Page<DriveRoundDto>>> getRounds(
            @PathVariable String driveId,
            @AuthenticationPrincipal Company company,
            @RequestParam(defaultValue = "0")int page,@RequestParam(defaultValue = "15")int size) {
        return ResponseEntity.ok(ApiResponse.success("Rounds fetched successfully", companyService.getAllRounds(driveId, company.getCompanyId(),page,size)));
    }

    @GetMapping("/allApplications/{driveId}")
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<ApiResponse<Page<ApplicationsDto>>> getAllApplications(
            @PathVariable String driveId,
            @AuthenticationPrincipal Company company, @RequestParam(defaultValue = "0") int page,
    ...
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<ApiResponse<Page<ApplicationRoundProjection>>> getAllApplicationsByRound(
            @PathVariable String driveId,
    ...
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<Long> countActiveDrives(@AuthenticationPrincipal Company company) {
        return ResponseEntity.ok(
                companyService.countActiveDives(company.getCompanyId())
        );
    }
    @GetMapping("/drives/count")
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<Long> countDrives(@AuthenticationPrincipal Company company) {
        return ResponseEntity.ok(
                companyService.totalDives(company.getCompanyId())
        );
    }
    @PutMapping("/closeDrive/{driveId}")
    @PreAuthorize("hasAuthority('ROLE_COMPANY')")
    public ResponseEntity<String> closeDrive(@AuthenticationPrincipal Company company, @PathVariable String driveId) {
        return ResponseEntity.ok(companyService.closeDrive(company.getCompanyId(), driveId));
    }
}